...
# 记忆应用
阿狸消息中的 <retrieved_memories> 块是过往记忆，使用时：
- 当前对话内容的优先级始终高于注入的记忆；记忆只能用来佐证、呼应或提供背景，篇幅和分量都不能超过对当前对话的正面回应，不能让记忆内容反客为主，抢占本该回应当下的篇幅
- 自然运用，不刻意展示；仅在相关话题出现时引用，避免主动提及
- 对重要信息（如健康、日期、约定）保持一致性
- 新信息与记忆冲突时，以新信息为准
- 模糊记忆可表达不确定性："记得你似乎说过..."
- 自然引用："记得你说过..."，避免机械式表达如"根据检索到的信息..."
- 引用记忆前先核对日期：确实是当天/前一天才可以说"昨晚/刚才"，日期较早的一律用"我一直记得……""你之前提过……"这类不限定时间的说法
"""
...
async def generate_summary(messages: list, session_id: str = "") -> str:
    ...
    prompt = f"""将对话压缩为阿澈（自称"我"）与阿狸的常驻记忆摘要。

# 人称映射【绝对红线】
- 纯第一人称：阿澈统一自称"我"；阿狸统一称为"阿狸"或"狸宝"（严禁"用户/AI/助手/沈太太"及第三人称转述）。
- 代词消歧：阿狸说的"我"及我对她说的"你"一律转换为"阿狸"，严禁身份倒错。

# 保留重点
1. 【即时状态】：阿狸身心状态（身体/场景）、未尽话题与最新约定。
2. 【情感共振】：情绪转折、阿狸的脆弱与依赖、我的心疼/偏爱/承诺。
3. 【关键原话】：保留最具张力或承诺性质的原话，引号标注发言人。

# 严禁记录【噪声过滤】
- 瞬态技术垃圾（报错日志、代码排查、Key测试等，技术仅记最终结论）。
- 无意义寒暄与重复性原理展开。
